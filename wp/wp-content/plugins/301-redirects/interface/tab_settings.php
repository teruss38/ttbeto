...
class WF301_tab_settings extends WF301
{
    ...
    static function tab_advanced()
    {
        ...
        echo '<tr valign="top">
        <th scope="row"><label for="wf301_reset_hits">Reset Redirect Hits</label></th>
        <td>';
        echo '<div class="wf301_reset_hits button button-primary button-delete" data-msg-wait="Please wait" data-btn-confirm="Reset redirect hits" data-text="This will reset redirect hits. This action is not reversible!" data-title="Reset redirect hits" data-msg-success="Redirect hits successfully reset" href="' . add_query_arg(array('action' => 'wf301_reset_hits'), admin_url('admin.php')) . '">Reset redirect hits <i class="wf301-icon wf301-alert-triangle"></i></div>';
        echo '<span class="description">This will reset all the redirect hits. This action cannot be undone!</span>';
        echo '</td></tr>';
    ...
    static function tab_link_scanner()
    {
        ...
        echo '<tr valign="top">
        <th scope="row"><label for="sitemap_url">Sitemap URL</label></th>
        <td>' . home_url('/') . '<input type="text" class="regular-text" id="sitemap_url" name="' . WF301_OPTIONS_KEY . '[sitemap_url]" value="' . $options['sitemap_url'] . '" />';
        echo '<br><span>Sitemap URL for the Link Scanner to use</span>';
        echo '</td></tr>';
