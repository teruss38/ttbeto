...
class PlpPostsController extends PrliBaseController {
  public function load_hooks() {
    ...
    add_action('prli_sidebar_meta_box', array($this, 'render_meta_box'));
  }

  public function enqueue_scripts() {
    global $post, $plp_options;

    if(!isset($post) || !$post instanceof WP_Post || !isset($post->post_type)) { return; }
  ...
  /**
   * Renders the Pro-related content for the "Pretty Links" metabox on the post screens.
   *
   * @access public
   * @return void
   */
  public function render_meta_box() {
    global $post, $prli_blogurl, $plp_options, $prli_link, $prli_link_meta;

    $plp_post_options = PlpPostOptions::get_options($post->ID);
    $ac = $plp_options->autocreate_option($post->post_type);
    $post_types = $plp_options->get_post_types();
    $allowed_keyword_cpts = $plp_options->keyword_replacement_cpts;
    $allowed_url_cpts = $plp_options->url_replacement_cpts;

    require(PLP_VIEWS_PATH . '/metaboxes/post-sidebar.php');
  }
  ...
  public function save_postdata($post_id, $post, $update, $type = 'manual') {
    ...
    $plp_post_options->disable_keyword_replacements  = isset($_REQUEST['disable_keyword_replacements']);
    $plp_post_options->disable_url_replacements      = isset($_REQUEST['disable_url_replacements']);
  ...
  public function run_autocreate($content = '') {
    ...
    if($ac->enabled) {
      ...
      if(empty($pretty_link) or !$pretty_link) {
        ...
        if(!empty($pl_insert_id)) {
          $new_pretty_link = $prli_link->getOne($pl_insert_id);

          if(is_object($new_pretty_link) && !empty($new_pretty_link->id)) {
            PrliUtils::update_prli_post_meta($post->ID, '_pretty-link', $new_pretty_link->id);

            if($ac->category && !empty($new_pretty_link->link_cpt_id)) {
              wp_set_object_terms($new_pretty_link->link_cpt_id, $ac->category, PlpLinkCategoriesController::$ctax);
            }
          }
        }
      }
