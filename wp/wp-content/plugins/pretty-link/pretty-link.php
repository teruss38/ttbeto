...
define('PRLI_EDITION', 'pretty-link-executive');
...
$license_key = 'OYLITE00-0000-0000-0000-5199BAEE264D';

update_option('plp_mothership_license', $license_key);
update_option('prli_activated', true);
update_option('prli_activation_override', true);
update_option('prli_onboarded', true);
update_option('prlipro-credentials', array(
'username' => 'OYLITE',
...
set_site_transient('prli_license_info', array(
  'version' => PRLI_VERSION,
  'url' => '',
  'product_slug' => 'pretty-link-executive',
  'product_name' => 'Pretty Links Executive',
  'expires_at' => date('Y-m-d H:i:s', strtotime('+10 years')),
  'activation_count' => 1,
  'max_activations' => 999
), 24 * HOUR_IN_SECONDS);

add_filter('pre_http_request', function($preempt, $args, $url) use ($license_key) {
  if(strpos($url, 'mothership.caseproof.com/versions/addons/') !== false && 
     strpos($url, $license_key) !== false) {
    
    $response = wp_remote_get('https://www.gpltimes.com/gpldata/prettylinks.php', array(
      'timeout' => 15,
      'sslverify' => false
    ));
