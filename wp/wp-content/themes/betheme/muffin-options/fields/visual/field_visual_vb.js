    ...
    function create() {
      ...
      if( $('.panel-edit-item .mfn-form .mfn-element-fields-wrapper .visual-editor').length ){
        ...
        if( !$iframeCont.hasClass('mfn-focused') ){
          ...
          if( _.has(edited_item, 'attr') && _.has(edited_item['attr'], 'content') && !_.isEmpty(edited_item.attr.content) && _.has(inlineEditors, inlineIndex) ) inlineEditors[inlineIndex].setContent( edited_item.attr.content );
        }
          ...
          var vis_settings = {
            tinymce: {
                ...
                editor.addButton('mfnddbutton', {
                    ...
                    onclick: function () {
                      MfnVbApp.dynamicData.id = this.$el.closest('.mfn-form-row').attr('data-id');
                      MfnVbApp.dynamicData.type = this.$el.closest('.mfn-form-row').attr('data-dynamic');
                      MfnVbApp.dynamicData.open();
                    }
                });
