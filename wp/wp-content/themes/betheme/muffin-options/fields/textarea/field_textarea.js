        ...
        __editor.instance[id].codemirror.on('change', function(cm, change) {
          ...
          /*if( id == 'custom-css-0' ) {
            ...
            if( localStorage.getItem("be_live_css_history") ) {
                ...
                if( helpr && !_.has(helpr, 'global_css') ) return;
                if( !helpr['global_css'] || !helpr['global_css'].length ) return;

                helpr['global_css'].push( cm.getValue() );

                localStorage.setItem("be_live_css_history", JSON.stringify(helpr));
            }
