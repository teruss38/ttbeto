...
/**
 * Betheme integration class for Yoast SEO Premium.
*/
...
class Yoast_Premium {
	 ...
	 * @var string
	 */
	public const SCRIPT_HANDLE = 'mfn-yoast-betheme';
	...
	*/
	...
	public function __construct() {
		$this->prominent_words_helper = new Prominent_Words_Helper( new Options_Helper() );
		$this->post_watcher           = new WPSEO_Post_Watcher();
		$this->current_page_helper    = new Current_Page_Helper();

		$this->arr_script = array(
			//'ai-generator' => 'ai-generator',
			//"ai-generator" => "ai-generator",
			//"ai-fix-assessments" => "ai-fix-assessments",
		);
	}
	...
	public function enqueue() {
		...
		// Re-register assets as Betheme unregister everything.
		$asset_manager = new WPSEO_Premium_Assets();
		$asset_manager->register_assets();

		$assets_manager = new WPSEO_Admin_Asset_Manager();

		$assets_manager->enqueue_style( 'premium-tailwind' );
		$assets_manager->enqueue_style( 'ai-fix-assessments' );
		$assets_manager->enqueue_style( 'premium-metabox' );

		$url = plugin_dir_url( WPSEO_PREMIUM_FILE );
	...
	public function send_data_to_assets() {
		...
		wp_localize_script( 'mfn-yoast-betheme', 'wpseoPremiumMetaboxData', [ 'data' => $data ] );


		foreach ($this->arr_script as $id => $script) {
			$id_name = "mfn-yoast-premium-".$id;
			wp_enqueue_script( $id_name, get_theme_file_uri('/visual-builder/integrations/yoast/js/dist/'.$script.'.js'), array(), time(), true );
		}
